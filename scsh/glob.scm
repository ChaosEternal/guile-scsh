...
(define (file-name-as-directory fn)
  (string-append fn "/"))

(define (filter-dot-files dotfile?)
  (lambda (x)
...
(define (directory-files dir dotfile?)
  (filter (filter-dot-files dotfile?) (port->list readdir (opendir dir))))
...
(define (glob-subpat fname pat) ; PAT doesn't contain a slash.
  (cond ((string=? pat "") (values '() #t))
	...
	(else (let* ((dots? (char=? #\. (string-ref pat 0))) ; Match dot files?
		     ...
		     (re (glob->regexp pat)))
		(values (filter (lambda (f) (string-match re f)) candidates)
...
(define glob->regexp-list
  (let ((dot-star ".*")) ; ".*" or (* any)
	...
	(let lp ((chars '())
		 ...
		 (i 0))
	  (if (= i pat-len)
	      (reverse (cons re-eos (str-cons chars res)))

	      (let ((c (string-ref pat i))
		    (i (+ i 1)))
		(case c
		  ((#\\) (if (< i pat-len)
			     (lp (cons (string-ref pat i) (cons #\\ chars))
				 res (+ i 1))
  ...
  (let ((pat-len (string-length pat)))
    (let lp ((elts '(#\[)) (i i))
	  ...
	  (let ((c (string-ref pat i))
	    ...
	    (case c
	      ((#\])
	       (values (list->string (reverse (cons #\] elts)))
		       i))
	      ((#\\)
	       (if (>= i pat-len)
		   (error "Ill-formed glob patten -- ends in backslash" pat)
		   (lp (cons (string-ref pat i) 
			     (cons #\\ 
				   elts))
		       (+ 1 i))))
